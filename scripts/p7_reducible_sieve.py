...
def coords():
 d={}
 for line in Path('research/beal/p7_rayclass_coords.csv').read_text().splitlines()[1:]:
  l,e1,e2,n=line.split(',');d[int(l)]=(int(e1),int(e2),int(n))
 return d
def main():
 alive={(k1,k2) for k1 in range(0,48,4) for k2 in (0,24)};co=coords();print('G',G,'initial',len(alive))
 for l in PRIMES:
  e1,e2,n=co[l];F=set()
  tv=tvals(l)
  for t in tv:
